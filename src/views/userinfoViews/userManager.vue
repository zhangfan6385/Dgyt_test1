<template>
    ...
    <div class="filter-container">
      ...
      style="width: 100%">
      <el-table-column align="center" :label="$t('userTable.userId')" width="65">
        <template slot-scope="scope">
          <span>{{scope.row.userId}}</span>
        </template>
      </el-table-column>
      <el-table-column width="150px" align="center" :label="$t('userTable.regTime')">
        <template slot-scope="scope">
     ...
     <el-table-column width="110px" align="center" :label="$t('userTable.userPass')">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="$t('userTable.actions')" width="230" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.status!='published'" size="mini" type="success" @click="handleModifyStatus(scope.row,'published')">{{$t('userTable.publish')}}
          </el-button>
          <el-button v-if="scope.row.status!='draft'" size="mini" @click="handleModifyStatus(scope.row,'draft')">{{$t('userTable.draft')}}
          </el-button>
          <el-button v-if="scope.row.status!='deleted'" size="mini" type="danger" @click="handleModifyStatus(scope.row,'deleted')">{{$t('userTable.delete')}}
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
     ...
     <el-form-item :label="$t('userTable.userCode')" prop="userCode">
        ...
        <el-form-item :label="$t('userTable.regTime')" prop="regTime">
          <el-date-picker v-model="temp.regTime" type="datetime" placeholder="Please pick a date">
          </el-date-picker>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      sortOptions: [{ label: '正序', key: '+userId' }, { label: '倒序', key: '-userId' }],
      showReviewer: false,
      temp: {
       userId: undefined,
        regTime: new Date(),
      ...
      rules: {
        ...
        userCode: [
          { required: true, message: "userCode is required", trigger: "blur" }
        ]
      },
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import("@/vendor/Export2Excel").then(excel => {
        const tHeader = ["timestamp", "title", "type", "flag", "status"];
        const filterVal = [
          "timestamp",
          "title",
          "type",
